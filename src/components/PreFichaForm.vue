<script setup>
import { camel2title } from '../assets/utils.js'
import useSteps from '../assets/useSteps'
...
<template>
    ...
    <FormKit type="form" #default="{ value, state: { valid } }" :plugins="[stepPlugin]" @submit="submitApp"
        ...
        <div class="form-body">
            <section v-show="activeStep === 'dadosPessoais'">
                <FormKit type="group" id="dadosPessoais" name="dadosPessoais">
                    <FormKit type="text" label="*Nome Completo" name="full_name" placeholder="Nome Completo"
                        validation="required" />

                    <FormKit type="email" name="email" label="*Email" placeholder="email@domain.com"
                        validation="required|email" />

                    <FormKit type="tel" name="tel" label="*Contato(whatsapp)" placeholder="(xx) xxxxx-xxxx"
                        help="O número de telefone deve ser no formato (xx) xxxxx-xxxx"
            ...
            <section v-show="activeStep === 'organizationInfo'">
                <FormKit id="organizationInfo" type="group" name="organizationInfo">
                    <FormKit type="text" label="*Organization name" name="org_name" placeholder="MyOrg, Inc."
                        help="Enter your official organization name." validation="required|length:3" />

                    <FormKit type="date" label="Date of incorporation"
                        :validation="[['before_date', new Date(Date.now())]]" name="date_inc" />
                </FormKit>
            </section>

            <section v-show="activeStep === 'application'">
                <FormKit id="application" type="group" name="application">
                    <FormKit type="checkbox" label="*I'm not a previous grant recipient"
                        help="Have you received a grant from us before?" name="not_previous_recipient"
                        validation="required|accepted" :validation-messages="{
                            accepted: 'We can only give one grant per organization.'
                        }" />
                    <FormKit type="textarea" label="*How will you use the money?" name="how_money"
